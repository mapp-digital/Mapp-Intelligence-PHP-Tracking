 ...
 */
class MappIntelligenceConsumerFileTest extends PHPUnit\Framework\TestCase
{
    private $longText;
    ...
    public function __construct($name = null, array $data = array(), $dataName = '')
    {
        ...
        $this->tempFilename = __DIR__ . '/../../tmp/mapp_intelligence_test.log';
        $this->longText = '';
    ...
    {
        new MappIntelligenceConsumerFile(array(
            'filename' => __DIR__ . '/../../../tmp/mapp_intelligence_test.log',
            'debug' => true
        ));

        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContains('failed to open stream: No such file or directory', $fileContent[0]);
    }

    public function testFailedToSendBatch()
    {
        $consumer = new MappIntelligenceConsumerFile(array(
            'filename' => __DIR__ . '/../../../tmp/mapp_intelligence_test.log',
            'debug' => true
        ));
    ...
    public function testMaxPayloadSize()
    {
        ...
        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContains('Payload size is larger than 24MB (34.7MB)', $fileContent[0]);
    }
    ...
    public function testWriteBatchRequestInNewFile()
    {
        ...
        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContains('Write batch data in ' . $this->tempFilename . ' (1 req.)', $fileContent[0]);
        $this->assertEquals("wt?p=300,0\n", file_get_contents($this->tempFilename));
    }

    public function testWriteBatchRequestInExistingNewFile()
    {
        $consumer = new MappIntelligenceConsumerFile(array(
            'filename' => $this->tempFilename,
            'debug' => true
        ...
        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContains('Write batch data in ' . $this->tempFilename . ' (3 req.)', $fileContent[0]);
        $this->assertContains('Write batch data in ' . $this->tempFilename . ' (2 req.)', $fileContent[1]);
        $this->assertEquals(
            "wt?p=300,0\nwt?p=300,1\nwt?p=300,2\nwt?p=300,3\nwt?p=300,4\n",
            file_get_contents($this->tempFilename)
        );
