 ...
 */
abstract class MappIntelligenceConsumerFileTestCase extends MappIntelligenceExtendsTestCase
{
    private $longText;
    ...
    public function __construct($tempFilename = '', $name = null, array $data = array(), $dataName = '')
    {
    ...
    public function testNewConsumerFile()
    {
        ...
        $this->assertContainsExtended('failed to open stream: no such file or directory', strtolower($fileContent[0]));
    }
    ...
    public function testWriteBatchRequestInNewFile()
    {
        ...
        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContainsExtended('Write batch data in ' . $this->tempFilename . ' (1 req.)', $fileContent[0]);
    ...
    public function testWriteBatchRequestInExistingNewFile()
    {
        ...
        $fileContent = MappIntelligenceUnitUtil::getErrorLog();
        $this->assertContainsExtended('Write batch data in ' . $this->tempFilename . ' (3 req.)', $fileContent[0]);
        $this->assertContainsExtended('Write batch data in ' . $this->tempFilename . ' (2 req.)', $fileContent[1]);
        $this->assertEquals(
            "wt?p=300,0\nwt?p=300,1\nwt?p=300,2\nwt?p=300,3\nwt?p=300,4\n",
            file_get_contents($this->tempFilename)
        );
